...
GOST_FONT_PATH = os.path.join(BASE_DIR, "fonts", "GOST typ A naklon.ttf")

def load_gost_font(size):
    return ImageFont.truetype(GOST_FONT_PATH, size)
...
def overlay_hoist_dimensions(mech_result, template_path, out_path="Eskiz-barbana_out.png", company_name=""):
    ...
    coords = {
        key: (int(rx * w), int(ry * h))
        for key, (rx, ry) in rel_coords.items()
    }
    ...
    if pipe is not None:
        ...
        steel = pipe["steel"]
        gost  = pipe["gost"]
        txt_material = f"{D_out:.0f}x{S:.0f}"
    else:
    ...
    # Рисуем все подписи
    draw_text_with_bg(draw, coords["L_b"],      txt_L_b,      FONT_GOST)
    draw_text_with_bg(draw, coords["L_h"],      txt_L_h,      FONT_GOST)
    draw_text_with_bg(draw, coords["L_g"],      txt_L_g,      FONT_GOST)
    draw_text_with_bg(draw, coords["L_k"],      txt_L_k,      FONT_GOST)
    # D_c и D_min — повернутые на 90°
    draw_rotated_text_with_bg(img, coords["D_c"],   txt_D_c,   FONT_GOST, angle=90)
    draw_rotated_text_with_bg(img, coords["D_min"], txt_D_min, FONT_GOST, angle=90)
    draw_text_with_bg(draw, coords["d_k"],      txt_d_k,      FONT_GOST)
    draw_text_with_bg(draw, coords["t"],      txt_t,      FONT_GOST)
    draw_text_with_bg(draw, coords["Zrt"],      txt_Zrt,      FONT_GOST)
    draw_text_with_bg(draw, coords["material"], txt_material, FONT_GOST)
    draw_text_with_bg(draw, coords["basic_box"], txt_basic,   FONT_GOST)
    draw_text_with_bg(draw, coords["polispast"], txt_polispast,   FONT_GOST)

    if txt_pipe_mass:
        draw_text_with_bg(draw, coords["pipe_mass"], txt_pipe_mass, FONT_GOST)

    draw_text_with_bg(draw, coords["company"], txt_company, FONT_GOST)
